...
from audithunter.models import Recommandations
...
def insert_recommandation(data):
    try:
        recommandation = ''.join(map(str, data.get('Recommandation', []))) if data.get('Recommandation') else None
        vulnerability = ''.join(map(str, data.get('Vulnerabilit_', []))) if data.get('Vulnerabilit_') else None
        impact = ''.join(map(str, data.get('Impact', []))) if data.get('Impact') else None
        probability = ''.join(map(str, data.get('Probabilit_', []))) if data.get('Probabilit_') else None
        ...
        recommandation = Recommandations(
            titre_reco=data.get('Titre_Reco', ''),
            recommandation=recommandation,
            titre_vuln=data.get('Titre_Vuln', ''),
            vuln=vulnerability,
            v_impact=impact,
            v_proba=probability,
            referents=referents,
